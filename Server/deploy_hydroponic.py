...
SRC = Path(r"C:\Users\carlo\OneDrive\Documents\Web Project\Smart Hydroponic Ciul")
REMOTE = "/home/carlo/smarthydroponic"
HOST_PORT = 3000
CONTAINER_PORT = 3000
EXCLUDE = {"node_modules", ".next", "build", "dist", ".git", "__pycache__", "data", ".cache", ".tmp", ".venv"}

def make_archive():
    archive = Path(tempfile.gettempdir()) / "hydroponic.tar.gz"
...
print("Uploading...")
sftp = c.open_sftp()
sftp.put(str(archive), "/tmp/hydroponic.tar.gz")
sftp.close()
archive.unlink()
...
run(f"mkdir -p {REMOTE} && tar -xzf /tmp/hydroponic.tar.gz -C {REMOTE} && rm /tmp/hydroponic.tar.gz && echo OK", 120)
compose = f"""services:
  hydroponic:
    build: .
    container_name: hydroponic
    ports:
...
run("docker ps --filter name=hydroponic --format '{{.Names}} {{.Status}} {{.Ports}}'", 30)
c.close()
print("DONE")
